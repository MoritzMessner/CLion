...
std::string Person::validateLastName(std::string _last_name) const {
    if (_last_name.empty())
        throw std::invalid_argument("falscher lastname");
    return _last_name;
};


Person::Person(std::string _first_name, std::string _last_name, Date _born) : last_name(
        validateLastName(_last_name)), first_name(
        validateFirstName(_first_name)), born(_born) {
