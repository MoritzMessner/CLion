...
namespace aufgabeSeven {


    void test_Vector() {
        auto v = aufgabeSeven::Vector3D(1, 1, 1);
        ...
        */
        ...
        // +
        auto x = m + v;
        assert(x.getX() == (v.getX() + m.getX()) && x.getY() == (v.getY() + m.getY()) &&
               x.getZ() == (v.getZ() + m.getZ()));
        auto y = 2 + m;
    ...
    void test_2() {
        ...
        ArrayList a(pString, 2);
        std::cout << a << std::endl;


        //b)
        // Überladung ist nötig, wenn ein Kopier Konstruktor(initializing) vorhanden ist, wir müssen auch korrekt bei
        // Zuweisung richtig kopieren. (wenn dynamische daten vorhanden sind)

        ArrayList b;
        b = a;
        b.set_name_on_position(1, "Axel");
        std::cout << a << std::endl;
        std::cout << b << std::endl;
